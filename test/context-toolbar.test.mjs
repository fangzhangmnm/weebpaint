...
describe("context-toolbar · 登记表 = index.html 顶栏条全集", () => {
  it("index.html 里的每条顶栏条都登记了（owner init 各一行；anchored-popup 不再手填 id）", async () => {
    ...
    assert(inHtml.length >= 3, "index.html 应有 ≥3 条静态顶栏条（套索/透视/裁切；滤镜笔/吸色/形状条由工厂生成——形状条 2026-09-18 回滚复活为工厂条 shapeToolbar），实得 " + inHtml.length);
    ...
    // 形状条 shapeToolbar 2026-09-18 回滚复活（工厂 mount，init 即登记）；几何条 rulerToolbar 随 ADR-0013 回滚删除，不得再出现
    assert(registered.has("filterBrushToolbar") && registered.has("pickerToolbar") && registered.has("shapeToolbar") && !registered.has("rulerToolbar"), "工厂 mount 的滤镜笔条/吸色条/形状条在登记表（init 即 mount）；几何条已删：" + [...registered].join(","));
  });
  // 2026-09-11 叠放归工厂（user「几何对齐还会不小心变成右对齐，然后换 context 的时候会突然空出来一大堆白」「应该走的是同一套代码」）：
  //   owner 不许再自己量别人的 bottom 写 top、不许右对齐特例——2026-09-18 回滚复活的形状条（shape-toolbar.ts）同样只 mountContextToolbar + show/hide。
  it("多条叠放只在工厂：shape-toolbar 不算 top / 不 ct-tail；styles 无 .ct-tail；工厂导出 relayoutContextToolbars", async () => {
    const owner = readFileSync(new URL("../src/shape-toolbar.ts", import.meta.url), "utf-8");
    assert(!/style\.top\s*=/.test(owner), "shape-toolbar 不该写 el.style.top");
    assert(!/classList\.add\("ct-tail"\)|contextToolbarBottomExcept\(/.test(owner), "shape-toolbar 不该挂 ct-tail / 调 contextToolbarBottomExcept");
