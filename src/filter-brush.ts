...
//   beginBrushStroke(targets, params, brushSettings, selection, x, y, pressure) → state
...
//
// targets（2026-08-28）= 写靶叶**列表**：单叶恒 [leaf]；active 是图层组且 filter 声明
...
export class FilterBrushEngine {
  ...
  beginStroke(targets: readonly ViewLeaf[], Filter: BrushFilter, params: unknown, brushSettings: unknown, selection: Selection | null, x: number, y: number, pressure: number) {
    if (!Filter || !Filter.beginBrushStroke) {
      throw new Error(`Filter ${Filter && Filter.id} does not support brush mode`);
    }
    if (!targets.length) throw new Error(`Filter ${Filter.id}: no target leaf`);
    if (targets.length > 1 && !Filter.supportsLayerGroup) {
      throw new Error(`Filter ${Filter.id} does not support layer groups (got ${targets.length} targets)`);
    }
    this._Filter = Filter;
    this._handle = Filter.beginBrushStroke(targets, params, brushSettings, selection, x, y, pressure);
  }
