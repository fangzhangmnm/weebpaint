...
| U7（修订 ④） | v0.14.5 | 手指位子工具改带图标下拉 `#filterBrushSubSel`（六项同 `finger` 图标；user「图标太打哑谜」「就是手指就行」）；`SelectItem.icon` + popup-menu compact 画 icon；左段 `26vw` 撤（宽屏空白病根）；sprite 陈旧守卫。edited by Claude Fable 5.1 2026-09-09 |
