...
> as-of v0.14.2 / 2026-09-07 · **本便条只有静态读码（grep + 读源），一行测试都没跑**——下面标 ✔/✘ 的是「代码写的是什么」，不是「实机验过什么」。
...
## 待讨论（user 2026-09-07 追加：「到时候我可以和那个agent讨论defringe的范围有多大，比如是不是每个图层都defringe」）——**先讨论再动码，别替 user 拍**

- **范围之一：哪些字节面**。现在只有导出 PNG；候选扩到 mergedimage.png / Blender push（上表），再到 **.ora 每层 `data/layer<id>.png`**（`src/backend/ora.ts` 215 逐层 encodePngFromBytes 处）。每层 defringe 的考量：layer PNG 是 authoring SSoT 不是预览件——α=0 处 RGB 变了对 WeebPaint 自己重开无影响的前提是「所有读 RGB 的地方都按 α 加权」；要盘一遍不按 α 加权采样 RGB 的路径（非预乘的模糊/涂抹/滤镜、吸色、重采样）会不会把回填色拉出来。别的 .ora reader（Krita/MyPaint）读层是 straight alpha，α=0 处 RGB 同样不可见。
- **范围之二：空间延拓多远**。现算法是全图多源 BFS 把整个透明区铺满边缘色；另一档是只铺 N px 环（mipmap 链越深要的环越宽，铺满最省心）。
- **范围之三：时机**。导出时一次性（现状、user 2026-08-19 拍板）vs 落盘时也做；每层做就等于半只脚进 authoring 管线，和「不进 lockAlpha」那条的边界要 user 划。
