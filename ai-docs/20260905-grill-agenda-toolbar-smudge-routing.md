...
### C-补：平流 formalism 是否让现有工具更优雅、少调参（2026-09-06 user 问「你先回答我数学上」；分析，非决策；实现归下一轮）

一句话：**是**——现有五个「沿笔迹推/搓/糊」的工具在连续形式下都是同一个方程的特例：

> 每帧对笔迹扫过区域做一次 `out(x) = Σ_k w_k(x) · src(x − d_k(x))`，再与 `orig` 按 mask 混。
> d = 位移场（沿笔迹切线，幅度 ∝ strength·mask），w = 采样核（δ=纯搬运，高斯=边搬运边糊）。

| 工具 | 现在的实现（离散 dab） | 连续形式 | 参数变化 |
|---|---|---|---|
| smear（手指推） | 每 dab 抓一块 accum 贴到下一处，`strength` 控混入、间距控拖尾长度、flowCoeff 补压感 | 位移场 d=切线×strength，核=δ | 间距、flowCoeff 消失；只剩 strength |
| dull（揉匀） | 每 dab 取 accum 与画布加权平均，`dull` 与间距耦合（密=糊得快） | 同一位移场，核=高斯 σ ∝ dull | 间距消失；dull 变成核宽，语义干净 |
| paint（带色手指） | 混 accum 里掺笔色 | 源 src 换成 `lerp(src, color, load)` | 不变，但与 smear 共享代码 |
| blur/sharpen 滤镜笔 | 每 dab 卷积一次；本轮改成 wash（覆盖 max + 一次 bake） | 位移 0、核=高斯（sharpen=负 unsharp） | wash 已经是连续形式的退化情形（d=0）——这次落地等于先铺了这条路 |
| liquify | 位移场累积一次重采样（已是连续形式） | 同一 warp 引擎 | 不变 |

所以调参会少掉：**间距/地板 10%、flowCoeff、accum 尺寸、dull-间距耦合**这四个「为离散化补洞的旋钮」，留下来的 strength、核宽、压感曲线都直接对应物理量。代价与需要衡量的：① 每帧对扫过 bbox 做一次 warp，成本 O(面积)，与笔径解耦（大笔更快、小笔略慢——阈值待测）；② smear 反复重采样会累积模糊，正解是像液化一样**累积位移场再一次重采样**（dull 反而是想要的）；③ 一笔内的时间顺序：位移场累积要按笔迹顺序写，回头描要有「后写覆盖」语义——这是 wash（max 覆盖）不需要处理的新坑；④ Krita 的 smudge 引擎参数（smudge length / smudge radius / color rate）可以直接映射到 strength / 核宽 / load，grounding 有现成对照。
结论：下一轮开「平流引擎」，先把 smear+dull 换心脏，blur wash 已就位，liquify 共用 warp；四个补洞旋钮随之退役。
...
- **智能填充（content-aware fill）**：user 2026-09-06「智能填充你忘了。和 B3 一样进 wishlist」——与上条 PatchMatch 同源（选区内按周围纹理合成），是修图族的第三步，不单列算法。
- **魔棒第三 option「容隙（形态学）」**：user 2026-09-06「gap 也记一下，做完另外一个 alternative，我们那个算法其实有时候蛮自作聪明的做魔棒的第三个 option 供用户选择。先不急着做」——排在湿画笔补全之后；= 现有 v0.7.24 容隙内核第 ③ 步升级为「细部整块归属」+ 升成具名下拉项。spec = `20260906-gap-closing-morphological-handoff.md`（clean-room，无代码）。
- **智能橡皮**（按边缘/颜色相似度自动停的擦）：user 2026-09-06「无限期搁置」——记在这里只为不再提，**不排期、不讨论**。
