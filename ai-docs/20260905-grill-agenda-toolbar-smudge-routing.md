...
| 「ui/context-toolbar 从登记表升级为 DOM 工厂深模块 同意」「子工具长按成标准件 同意」「单画笔位 + 长按出子工具 同意」「『…』溢出位 同意，是工具条的自带功能，不用特殊实现」「布局断点表赞」 | UI 抽象轮策划 `20260906-ui-abstraction-round-proposal.md`（现状 .h + 提案 .h，等「没问题」） |
